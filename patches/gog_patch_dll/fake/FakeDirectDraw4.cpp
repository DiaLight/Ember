...
HRESULT FakeDirectDraw4::CreateSurface(LPDDSURFACEDESC2 pDesc, LPDIRECTDRAWSURFACE4 *ppSurf, IUnknown *a4) {
  if (!pDesc)
    gog_debug("Assertion failed: FakeDirectDraw4::CreateSurface:1053");
  if (a4)
    gog_debug("Assertion failed: FakeDirectDraw4::CreateSurface:1054");
  auto *surf = new FakeSurface4(pDesc);
  if (!surf->orig())
    gog_debug("Assertion failed: FakeDirectDraw4::CreateSurface:1056");
  *ppSurf = surf;
  return DD_OK;
}
...
HRESULT FakeDirectDraw4::GetCaps(LPDDCAPS hwCaps, LPDDCAPS halCaps) {
  if ( !hwCaps )
    gog_debug("Assertion failed: FakeDirectDraw4::GetCaps:1065");
  if ( halCaps )
    gog_debug("Assertion failed: FakeDirectDraw4::GetCaps:1066");
  DDCAPS hwCaps_;
...
HRESULT FakeDirectDraw4::GetDeviceIdentifier(LPDDDEVICEIDENTIFIER pDevId, DWORD) {
  ...
  if (!pDevId)
    gog_debug("Assertion failed: FakeDirectDraw4::GetDeviceIdentifier:1126");
