...
bool patch::patch_draw_buffer_extra_size() {
    mydd_alloc_buf = (mydd_alloc_buf_t) (api::dk2_base + (0x005A7980 - dk2_virtual_base));
    mydd_alloc_buf2 = (mydd_alloc_buf_t) (api::dk2_base + (0x005A79C0 - dk2_virtual_base));
    if (!api::replaceXrefs((uint8_t *) mydd_alloc_buf, reimpl_mydd_alloc_buf)) return false;
    if (!api::replaceXrefs((uint8_t *) mydd_alloc_buf2, reimpl_mydd_alloc_buf2)) return false;
    return true;
}
