...
bool remove_mouse_dinput() {
  if(!replaceXrefs(funptr<&dk2::MyDxMouse::create>(), proxy_MyDxMouse_create)) return false;
  // don't attach dinput mouse to ControlKeysUpdater
  uint8_t *pos = dk2_base + (0x005BC091 - dk2_virtual_base);  // .text:005BC091    jge     short loc_5BC0A1
  {
    DWORD oldProtect;
...
void click_mouse(DWORD dik_scancode, DWORD flags) {
  auto *updater = dk2::MyInputManagerCb::getInstance()->f5C_controlKeys;
  // do not try to call constructor/destructor
  auto *action = (dk2::MouseRgbDxAction *) new char[sizeof(dk2::MouseRgbDxAction)];
  *(void **) action = funptr<&dk2::MouseRgbDxAction::vtable>();
//  action.f10_KeyCode_F0toF3 = click_dinput_to_dk2(dinput_dwOffs);
  action->f10_KeyCode_F0toF3 = dik_scancode;
  action->f20_btnPressFlags = flags;
  action->f14_pos.x = 0;
  action->f14_pos.y = 0;
  // action->f1C_data = LOBYTE(DIDEVICEOBJECTDATA.dwData)
  action->f1C_data = flags & DK2_IsPressed ? 0x80 : 0x00;
  action->f4_timestamp = GetTickCount();
  action->f8_isNotHandled = 1;
  actionsInProgress.push_back(action);
  updater->vtbl()->fun4(updater, NULL, action);
}
...
bool emulate_dinput_from_user32() {
  ...
  api::BEFORE_WINDOW_PROC.emplace_back([](HWND &hWnd, UINT &Msg, WPARAM &wParam, LPARAM &lParam) {
    switch(Msg) {
      case WM_LBUTTONDOWN: click_mouse(DIK_DK2_LEFTMOUSE, DK2_IsPressed | controlFlags); break;
      case WM_LBUTTONUP: click_mouse(DIK_DK2_LEFTMOUSE, 0 | controlFlags); break;
      case WM_LBUTTONDBLCLK: click_mouse(DIK_DK2_LEFTMOUSE, DK2_IsDblClick | controlFlags); break;
      case WM_RBUTTONDOWN: click_mouse(DIK_DK2_RIGHTMOUSE, DK2_IsPressed | controlFlags); break;
      case WM_RBUTTONUP: click_mouse(DIK_DK2_RIGHTMOUSE, 0 | controlFlags); break;
      case WM_RBUTTONDBLCLK: click_mouse(DIK_DK2_RIGHTMOUSE, DK2_IsDblClick | controlFlags); break;
      case WM_MBUTTONDOWN: click_mouse(DIK_DK2_MIDDLEMOUSE, DK2_IsPressed | controlFlags); break;
      case WM_MBUTTONUP: click_mouse(DIK_DK2_MIDDLEMOUSE, 0 | controlFlags); break;
      case WM_MBUTTONDBLCLK: click_mouse(DIK_DK2_MIDDLEMOUSE, DK2_IsDblClick | controlFlags); break;
      case WM_XBUTTONDOWN: click_mouse(DIK_DK2_UNKMOUSE, DK2_IsPressed | controlFlags); break;
      case WM_XBUTTONUP: click_mouse(DIK_DK2_UNKMOUSE, 0 | controlFlags); break;
      case WM_XBUTTONDBLCLK: click_mouse(DIK_DK2_UNKMOUSE, DK2_IsDblClick | controlFlags); break;
      case WM_KEYDOWN: {
        switch (wParam) {
          case VK_SHIFT: controlFlags |= DK2_Shift;
          case VK_CONTROL: controlFlags |= DK2_Ctrl;
          case VK_MENU: controlFlags |= DK2_Alt;
        }
        break;
      }
      case WM_KEYUP: {
