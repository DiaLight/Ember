...
void __fastcall proxy_fun3_xyzAction(dk2::MyMouse *this_, void *edx, dk2::MouseXyzDxAction &action) {
  ...
  if(lastMousePos_first) {
    ...
    HWND hWnd = dk2::MyGame::getInstance()->fEC9_c_window_test.f4_hWnd;
    ...
    if(action.fC_actedAxe == DIMOFS_X) {  // x axe
      float absX = (float) action.f10_value / (float) renderSize.x * (float) workBox.x;
      if(absX < clientBox.minX || clientBox.maxX <= absX) return;
      float dAbsX = absX - lastMousePos.x;
      printf("x: %d %.1f %.1f\n", action.f10_value, absX, dAbsX);
      action.f10_value = (int) dAbsX;
      lastMousePos.x = absX;
    } else if(action.fC_actedAxe == DIMOFS_Y) {  // y axe
      float absY = (float) action.f10_value / (float) renderSize.y * (float) workBox.y;
      if(absY < clientBox.minY || clientBox.maxY <= absY) return;
      float dAbsY = absY - lastMousePos.y;
      printf("y: %d %.1f %.1f\n", action.f10_value, absY, dAbsY);
      action.f10_value = (int) dAbsY;
      lastMousePos.y = absY;
    }
  }
