...
HRESULT FakeDirectDraw1::EnumDisplayModes(DWORD, LPDDSURFACEDESC pDesc, LPVOID a4, LPDDENUMMODESCALLBACK cb) {
  if (pDesc)
    gog_debug("Assertion failed: FakeDirectDraw1::EnumDisplayModes:1289");
  DDSURFACEDESC desc;
...
HRESULT FakeDirectDraw1::GetCaps(LPDDCAPS hwCaps, LPDDCAPS halCaps) {
  if ( !hwCaps )
    gog_debug("Assertion failed: FakeDirectDraw1::GetCaps:1370");
  if ( halCaps )
    gog_debug("Assertion failed: FakeDirectDraw1::GetCaps:1371");
  DDCAPS caps;
...
HRESULT FakeDirectDraw1::SetDisplayMode(DWORD width, DWORD height, DWORD a4) {
  ...
  g_dwHeight = height;
  if (a4 != 32)
    gog_debug("Assertion failed: FakeDirectDraw1::SetDisplayMode:1488");
  if (cfg::iNotOnTop)
