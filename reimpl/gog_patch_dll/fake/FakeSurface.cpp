...
FakeSurface::FakeSurface(DDSURFACEDESC2 *desc) {
  ...
  if ( (dwFlags & 0x40) != 0 || (descCpy.ddsCaps.dwCaps & 0x20000) != 0 )
    gog_debug("Assertion failed: FakeSurface::FakeSurface:437");
  memcpy(&this->f8_surfDesc, &descCpy, sizeof(this->f8_surfDesc));
  if ( !this->f8_surfDesc.dwWidth )
    gog_debug("Assertion failed: FakeSurface::FakeSurface:441");
  if ( !this->f8_surfDesc.dwHeight )
    gog_debug("Assertion failed: FakeSurface::FakeSurface:442");
  ...
  if (cfg::iAntialias && g_dwWidth <= 1024) {
    ...
    IDirectDrawSurface4 *ZBuf = (IDirectDrawSurface4 *)Fake_CreateZBuf(g_dwWidth * cfg::iAntialias, g_dwHeight * cfg::iAntialias);
...
HRESULT FakeSurface::AddAttachedSurface(LPDIRECTDRAWSURFACE2 a2) {
  if (!a2)
    gog_debug("Assertion failed: FakeSurface::AddAttachedSurface:539");
  if (this != FakeSurface::instance_cpy)
    gog_debug("Assertion failed: FakeSurface::AddAttachedSurface:540");
  return 0;
}
...
HRESULT FakeSurface::Flip(LPDIRECTDRAWSURFACE2 a2, DWORD flags) {
  if (!this->f84_isModSurf)
    gog_debug("Assertion failed: FakeSurface::Flip:606");
  if (!orig::pIDirectDrawSurface4_coop)
    gog_debug("Assertion failed: FakeSurface::Flip:607");
  if ( a2 && a2 != FakeSurface::instance_cpy)
    gog_debug("Assertion failed: FakeSurface::Flip:608");
  if (gog::g_isSceneDrawing)
    gog_debug("Assertion failed: FakeSurface::Flip:609");
  if (cfg::iCpuIdle)
    Sleep(cfg::iCpuIdle);
...
HRESULT FakeSurface::GetAttachedSurface(LPDDSCAPS a2, LPDIRECTDRAWSURFACE2 *a3) {
  if (!this->f84_isModSurf)
    gog_debug("Assertion failed: FakeSurface::GetAttachedSurface:635");
  if ((a2->dwCaps & 4) == 0)
    gog_debug("Assertion failed: FakeSurface::GetAttachedSurface:636");
...
HRESULT FakeSurface::Lock(LPRECT pRect, LPDDSURFACEDESC pDesc, DWORD a4, HANDLE a5) {
  if (g_isSmth_noRef && (this == FakeSurface::instance_mod || this == FakeSurface::instance_cpy))
    return DDERR_GENERIC;
  if (this->f9C_pLockedRect)
    gog_debug("Assertion failed: FakeSurface::Lock:672");
  DDSURFACEDESC2 desc;
  memset(&desc, 0, sizeof(desc));
  desc.dwSize = sizeof(DDSURFACEDESC2);
  static_assert(sizeof(DDSURFACEDESC2) == 124);
