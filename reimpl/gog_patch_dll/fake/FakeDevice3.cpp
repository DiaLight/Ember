...
FakeDevice3::FakeDevice3() {
  ...
  if (!cfg::iAnisotropy) {
    ...
    if (FAILED(hr)) {
      gog_debugf("Assertion failed: FakeDevice3::FakeDevice3:755 with HRESULT 0x%x\n");
    }
  }
}
...
HRESULT FakeDevice3::BeginScene(void) {
  ...
  if (gog::g_isFlip) {
    ...
    if (FAILED(hr))
      gog_debug("Assertion failed: FakeDevice3::BeginScene:837");
    g_isFlip = false;
  }
...
HRESULT FakeDevice3::SetRenderState(D3DRENDERSTATETYPE stateType, DWORD a3) {
  ...
  if ( stateType == D3DRENDERSTATE_ANTIALIAS )
    gog_debug("Assertion failed: FakeDevice3::SetRenderState:879");
...
HRESULT FakeDevice3::DrawIndexedPrimitive(D3DPRIMITIVETYPE primitive_type, DWORD fvf, LPVOID vertices, DWORD vertex_count, LPWORD indices, DWORD index_count, DWORD flags) {
  if (!g_isSceneDrawing)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:914");
  if (primitive_type != D3DPT_TRIANGLELIST)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:915");
  if (cfg::iBumpmap) {
    if (fvf != Vertex1C_TypeDesc && fvf != Vertex2C_TypeDesc)
      gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:917");
  } else if ( fvf != Vertex1C_TypeDesc ) {
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:919");
  }
  if (!vertices)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:921");
  if (!vertex_count)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:922");
  if (!indices)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:923");
  if (!index_count)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:924");
  if (flags != 0x1C)
    gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:925");
  ...
  if (fvf == Vertex1C_TypeDesc) {
    if (vertex_count) {
      ...
      do {
        if (pos->f8_z < 0.0 || pos->f8_z > 1.0)
          gog_debug("Assertion failed: FakeDevice3::DrawIndexedPrimitive:929");
        ++pos;
        --vertex_left;
      } while(vertex_left);
    }
...
HRESULT FakeDevice3::DrawIndexedPrimitiveStrided(D3DPRIMITIVETYPE, DWORD, LPD3DDRAWPRIMITIVESTRIDEDDATA, DWORD, LPWORD, DWORD, DWORD) {
...
HRESULT FakeDevice3::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value) {
  if (cfg::iBumpmap) {
    if (Stage != 0 && Stage != 1 && Stage != 2 )
      gog_debug("Assertion failed: FakeDevice3::SetTextureStageState:970");
  } else if (Stage != 0) {
    gog_debug("Assertion failed: FakeDevice3::SetTextureStageState:972");
  }
  switch (Type) {
    case D3DTSS_MAXANISOTROPY:
      gog_debug("Assertion failed: FakeDevice3::SetTextureStageState:975");
      break;
