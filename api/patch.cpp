...
void PatchBuilder::writeAsm(void (*asmFun)()) {
  ...
  if(*pos == 0xE9) {  // jump table. just follow
    size_t offs = *(uint32_t *) (++pos); pos += 4;
    pos += offs;
  }
  assert(*pos == 0xEB);
  size_t size = *(uint8_t *) (++pos); pos += 1;
  write(pos, size);
}
...
_declspec(naked) void saveRegs_asm() {
  _asm {
...
_declspec(naked) void loadRegs_asm() {
  _asm {
      jmp    end  // emit size of content [0xE9, offs to end]
      pop    eax
      pop    ebx
...
HookHandle *HookHandle::replaceCall(uint8_t *orig_addr, int clear_stack_args) {
  ...
  size_t proxy = pb.offs(); pb.write<uint32_t>(0xFFFFFFFF);
  pb.writeAsm(mov1_asm);
  pb.write<uint8_t>(0x68); pb.write<uint32_t>((uint32_t) target);  // push <uint32_t>
  pb.write<uint8_t>(0xE8);  // call ...
  size_t run = pb.offs(); pb.write<uint32_t>(0xFFFFFFFF);
  ...
  *(uint32_t *) (handle->code + run) = /*dst=*/((uint8_t *) &HookHandle::_call_proxy) - /*src=*/(handle->code + run + 4);
...
HookHandle *HookHandle::_create(uint8_t *orig_addr, size_t orig_size, size_t reloc_offs, bool overwrite) {
  ...
  size_t back = pb.offs(); pb.write<uint32_t>(0xFFFFFFFF);
